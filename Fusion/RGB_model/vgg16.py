...
#
# trainLoader = torch.utils.data.DataLoader(RGB_dataset=trainData, batch_size=BATCH_SIZE, shuffle=True)
# testLoader = torch.utils.data.DataLoader(RGB_dataset=testData, batch_size=BATCH_SIZE, shuffle=False)
...
class VGG16(tnn.Module):
    def __init__(self,k,n_classes=1000):
        super(VGG16, self).__init__()

        # Conv blocks (BatchNorm + ReLU activation added in each block)
        self.layer1 = vgg_conv_block([3*k, 64], [64, 64], [3, 3], [1, 1], 2, 2)
        ...
        # FC layers
        self.layer6 = vgg_fc_layer(3*3*512, 2048)
        self.layer7 = tnn.Linear(2048, n_classes)
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out= self.layer5(out)#32 512 3 3
