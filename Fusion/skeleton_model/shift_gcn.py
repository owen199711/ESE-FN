...
import sys
sys.path.append("./skeleton_model/Temporal_shift/")
...
class Model(nn.Module):
    def __init__(self,output_layers,num_class=55, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3):
        ...
        A = self.graph.A
        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point)# M,C,V
        ...
        self.l10 = TCN_GCN_unit(256, 256, A)  # ->change(256, 256, A)->(256, 512, A)
    ...
    def forward(self, x):
        ...
        x1=x.view(N,c_new,-1,2)
        x = x.mean(3).mean(1)

        x2=self.fc(x)
        out=[]
        out.append(x2)
        if self.output_layers=='more_layer1':
            # out.append(x1)
            out.append(x)
            return out
        else :
